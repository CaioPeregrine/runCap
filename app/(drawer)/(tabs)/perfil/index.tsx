...
// ── Substituição: importa o componente compartilhado ──────────────────────────
...
export default function Perfil() {
  ...
  // ── Derivados do XP para o XPProgressBar ─────────────────────────────────
  // xpInicioNivel: XP acumulado no começo do nível atual
  // xpFimNivel:    XP acumulado no fim do nível atual (= xpInicioNivel + 200)
  // nivelMaximo:   true quando o usuário atingiu o nível máximo
  const nivelMaximo   = nivel >= NIVEL_MAXIMO;
  const xpInicioNivel = nivelMaximo ? XP_MAXIMO - XP_POR_NIVEL : (nivel - 1) * XP_POR_NIVEL;
  const xpFimNivel    = nivelMaximo ? XP_MAXIMO                : nivel * XP_POR_NIVEL;
  // ─────────────────────────────────────────────────────────────────────────
  ...
  async function fetchDados() {
    try {
      ...
      setNivel(u?.nivel ?? 1);
      setXp(u?.xp ?? 0);
      // xpProximo removido: XPProgressBar calcula internamente via XP_POR_NIVEL
  ...
  return (
    <View style={styles.inicial}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }}>
        ...
        <View>
          ...
          <TouchableOpacity style={styles.bannerArea} onPress={escolherBackground} activeOpacity={0.85}>
            ...
            <View style={styles.bannerCamera}>
              {loadBg
                ? <ActivityIndicator size="small" color="#2C3F69" />
                : <Ionicons name="camera" size={15} color="#2C3F69" />}
            </View>
          </TouchableOpacity>
        ...
        {/* ══════════════════════════════════════════════════════════════
            BARRA DE XP — agora usa XPProgressBar compartilhado
            · xpTotal        = xp acumulado total do usuário
            · xpInicioNivel  = XP onde o nível atual começou
            · xpFimNivel     = XP onde o nível atual termina
            · nivel          = nível atual
            · nivelMaximo    = true se chegou no teto
        ══════════════════════════════════════════════════════════════ */}
        <View style={styles.secao}>
          <XPProgressBar
            xpTotal={xp}
            xpInicioNivel={xpInicioNivel}
            xpFimNivel={xpFimNivel}
            nivel={nivel}
        ...
        {/* ══════════════════════════════════════════════════════════════
            ESTATÍSTICAS
        ══════════════════════════════════════════════════════════════ */}
        <View style={{ marginHorizontal: 16, marginTop: 12, borderRadius: 16, paddingVertical: 16 }}>
